    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        ...
        <div className="card-surface p-6">
          {tab === "profile" ? <ProfileTab profile={profile} userId={user?.id ?? ""} email={user?.email ?? ""} onSaved={refreshProfile} /> : null}
          {tab === "workspace" ? <WorkspaceTab org={org} role={membership?.role ?? "member"} onSaved={refreshProfile} /> : null}
...
function ProfileTab({ profile, userId, email, onSaved }: any) {
  ...
  const hasChanges = useMemo(() => name.trim() !== (lastSavedName ?? "").trim(), [lastSavedName, name]);

  const save = async (nextName: string) => {
    setSaving(true);
    const { error } = await supabase.from("profiles").update({ full_name: nextName.trim() || null }).eq("id", userId);
    setSaving(false);
    if (error) {
    ...
    toast.success("Profile updated", {
      cancel: {
        label: "Cancel",
        onClick: async () => {
          const { error: revertError } = await supabase.from("profiles").update({ full_name: profile?.full_name ?? null }).eq("id", userId);
          if (revertError) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Full name"><input className="input-field" value={name} onChange={(event) => setName(event.target.value)} /></Field>
      <button onClick={() => save(name)} className="btn-primary inline-flex items-center gap-2" disabled={saving || !hasChanges}>
...
function WorkspaceTab({ org, role, onSaved }: any) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Name"><input className="input-field" value={name} onChange={(event) => setName(event.target.value)} disabled={!canEdit} /></Field>
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="New password">
        <input type="password" className="input-field" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" />
      </Field>
